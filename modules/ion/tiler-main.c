...
static struct tmm2 *tmm[TILER_FORMATS];
static u32 *dmac_va;
...
static s32 pin_mem_to_area(struct tmm2 *tmm, struct tcm_area *area, u32 *ptr)
{
	s32 res = 0;
	...
	tcm_for_each_slice(slice, *area, area_s) {
		...
		/* pin memory into DMM */
		if (tmm2_pin(tmm, p_area, dmac_pa)) {
			res = -EFAULT;
			break;
		}
	}
...
static void unpin_mem_from_area(struct tmm2 *tmm, struct tcm_area *area)
{
	struct pat_area p_area = {0};
	struct tcm_area slice, area_s;
...
static s32 __analize_area(enum tiler2_fmt fmt, u32 width, u32 height,
...
void fill_virt_array(struct tiler_block_t *blk, u32 *virt_array)
{
	...
	v = tiler2_vstride(blk);
	p = tiler2_pstride(blk);

	/* get page aligned virtual size for the block */
	size = tiler2_size(blk);
	offs = blk->phys;
	while (size) {
...
s32 tiler2_fill_virt_array(tiler_blk_handle handle, u32 *virt_array,
		u32 *array_size)
{
	...
	/* get page aligned virtual size for the block */
	size = PAGE_ALIGN(tiler2_size(blk));
...
EXPORT_SYMBOL(tiler2_fill_virt_array);
...
static s32 lay_2d(enum tiler2_fmt fmt, u16 n, u16 w, u16 h, u16 band,
...
static void _m_unpin(struct mem_info *mi)
{
	...
	if (mi->pa.memtype == TILER_MEM_GOT_PAGES) {
		...
		tmm2_free(tmm[tiler2_fmt(mi->blk.phys)], mi->pa.mem);
		/*
	...
	mi->pa.mem = NULL;
	mi->pa.num_pg = 0;
	unpin_mem_from_area(tmm[tiler2_fmt(mi->blk.phys)], &mi->area);
}
...
static struct mem_info *find_block_by_ssptr(u32 sys_addr)
{
	...
	list_for_each_entry(i, &blocks, global) {
		if (tiler2_fmt(i->blk.phys) == tiler2_fmt(sys_addr) &&
		    tcm_is_in(pt, i->area)) {
			i->refs++;
			goto found;
		}
...
static void fill_block_info(struct mem_info *i, struct tiler_block_info *blk)
{
	...
	if (blk->fmt == TILFMT_PAGE) {
		...
		blk->stride = tiler2_vstride(&i->blk);
 ...
 *  ==========================================================================
 */
static struct mem_info *alloc_area(enum tiler2_fmt fmt, u32 width, u32 height,
				   struct gid_info *gi)
{
	u16 x, y, band, align;
	struct mem_info *mi = NULL;
...
static struct mem_info *alloc_block_area(enum tiler2_fmt fmt, u32 width,
...
{
	enum tiler2_fmt fmt = tiler2_fmt(mi->blk.phys);
	struct tcm_area area = mi->area;

	/* ensure we can pin */
	if (!tmm2_can_pin(tmm[fmt]))
		return -EINVAL;
...
void tiler2_pa_free(struct tiler_pa_info *pa)
{
	if (pa)
		kfree(pa->mem);
	kfree(pa);
}
EXPORT_SYMBOL(tiler2_pa_free);

/* allocate physical pages for a block */
static struct tiler_pa_info *get_new_pa(struct tmm2 *tmm, u32 num_pg)
{
...
static s32 alloc_block(enum tiler2_fmt fmt, u32 width, u32 height,
...
struct tiler_pa_info *user_block_to_pa(u32 usr_addr, u32 num_pg)
{
	...
	for (i = 0; i < num_pg; i++) {
		...
		if (got_pg && get_user_pages(curr_task, mm, usr_addr, 1,
			...
			got_pg = 0;
			mem[i] = tiler2_virt2phys(usr_addr);
			if (!mem[i]) {
...
static s32 pin_any_block(enum tiler2_fmt fmt, u32 width, u32 height,
		     u32 key, u32 gid, struct process_info *pi,
...
static s32 pin_block(enum tiler2_fmt fmt, u32 width, u32 height,
		     u32 key, u32 gid, struct process_info *pi,
...
s32 tiler2_pin_block(tiler_blk_handle block, u32 *addr_array, u32 nents)
{
	struct tiler_pa_info *pa = NULL;
	u32 *mem = NULL;
	int res;
...
EXPORT_SYMBOL(tiler2_pin_block);
...
#ifdef CONFIG_PM
static int tiler2_resume(struct device *pdev)
{
	struct mem_info *mi;
	...
	tmm2_unpin(tmm[TILFMT_8BIT], area);

	/* iterate over all the blocks and refresh the PAT entries */
	list_for_each_entry(mi, &blocks, global) {
		if (mi->pa.mem)
			if (pin_mem_to_area(tmm[tiler2_fmt(mi->blk.phys)],
						&mi->area, mi->pa.mem))
...
static s32 __init tiler_init(void)
{
	...
	tmm_pat = tmm2_pat_init(0, dmac_va, dmac_pa);
	...
	if (tiler_major) {
		...
		r = register_chrdev_region(dev, 1, "tiler2");
	} else {
		r = alloc_chrdev_region(&dev, tiler_minor, 1, "tiler2");
	...
	tilerdev_class = class_create(THIS_MODULE, "tiler2");
	...
	device = device_create(tilerdev_class, NULL, dev, NULL, "tiler2");
	if (device == NULL)
	...
	dbgfs = debugfs_create_dir("tiler2", NULL);
...
tiler_blk_handle tiler2_map_1d_block(struct tiler_pa_info *pa)
{
	struct mem_info *mi = NULL;
...
EXPORT_SYMBOL(tiler2_map_1d_block);

void tiler2_free_block_area(tiler_blk_handle block)
{
	mutex_lock(&mtx);
	_m_try_free(block);
	mutex_unlock(&mtx);
}
EXPORT_SYMBOL(tiler2_free_block_area);

tiler_blk_handle tiler2_alloc_block_area(enum tiler2_fmt fmt, u32 width,
...
EXPORT_SYMBOL(tiler2_alloc_block_area);

void tiler2_unpin_block(tiler_blk_handle block)
{
	mutex_lock(&mtx);
	_m_unpin(block);
	mutex_unlock(&mtx);
}
EXPORT_SYMBOL(tiler2_unpin_block);

s32 tiler2_memsize(enum tiler2_fmt fmt, u32 width, u32 height, u32 *alloc_pages,
		  u32 *virt_pages)
{
	u16 x, y, band, align;
	int res;
	...
	*alloc_pages = *virt_pages = 0;
	...
	if (!res) {
		...
		*alloc_pages = x*y;
		*virt_pages = tiler2_size(&blk) / PAGE_SIZE;
	}

	return res;
}
EXPORT_SYMBOL(tiler2_memsize);

u32 tiler2_block_vstride(tiler_blk_handle block)
{
	return tiler2_vstride(&block->blk);
}
EXPORT_SYMBOL(tiler2_block_vstride);
