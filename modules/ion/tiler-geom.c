...
#define TILER_FMT(x)	((enum tiler2_fmt) \
...
bool is_tiler2_addr(u32 phys)
{
	return phys >= TILVIEW_8BIT && phys < TILVIEW_END;
}
EXPORT_SYMBOL(is_tiler2_addr);

u32 tiler2_bpp(const struct tiler_block_t *b)
{
	enum tiler2_fmt fmt = tiler2_fmt(b->phys);
	BUG_ON(fmt == TILFMT_INVALID);

	return geom[fmt].bpp_m;
}
EXPORT_SYMBOL(tiler2_bpp);

/* return the stride of a tiler-block in tiler space */
static inline s32 tiler2_stride(u32 tsptr)
{
	enum tiler2_fmt fmt = tiler2_fmt(tsptr);
...
u32 tiler2_pstride(const struct tiler_block_t *b)
{
	enum tiler2_fmt fmt = tiler2_fmt(b->phys);
	BUG_ON(fmt == TILFMT_INVALID);
	...
	if (fmt == TILFMT_PAGE)
		return tiler2_vstride(b);

	return tiler2_stride(b->phys & ~MASK_VIEW);
}
EXPORT_SYMBOL(tiler2_pstride);

enum tiler2_fmt tiler2_fmt(u32 phys)
{
	if (!is_tiler2_addr(phys))
		return TILFMT_INVALID;

	return TILER_FMT(phys);
}
EXPORT_SYMBOL(tiler2_fmt);

/* returns the tiler geometry information for a format */
static const struct tiler_geom *get_geom(enum tiler2_fmt fmt)
{
...
static u32 tiler_get_address(u32 orient, enum tiler2_fmt fmt, u32 x, u32 y)
{
...
void tilview_create(struct tiler_view_t *view, u32 phys, u32 width, u32 height)
{
	...
	view->height = height;
	view->h_inc = view->bpp;
	view->v_inc = tiler2_stride(view->tsptr);
}
...
static inline u32 alias_address(enum tiler2_fmt fmt, u32 x, u32 y)
{
