...
EXPORT_SYMBOL(tiler2_virt2phys);

void tiler2_reservex(u32 n, enum tiler2_fmt fmt, u32 width, u32 height,
		   u32 gid, pid_t pid)
{
...
EXPORT_SYMBOL(tiler2_reservex);

void tiler2_reserve(u32 n, enum tiler2_fmt fmt, u32 width, u32 height)
{
	tiler2_reservex(n, fmt, width, height, 0, current->tgid);
}
EXPORT_SYMBOL(tiler2_reserve);

#ifdef CONFIG_TILER_ENABLE_NV12
void tiler2_reservex_nv12(u32 n, u32 width, u32 height,
			u32 gid, pid_t pid)
{
	struct process_info *pi = __get_pi(pid, true);
...
EXPORT_SYMBOL(tiler2_reservex_nv12);

void tiler2_reserve_nv12(u32 n, u32 width, u32 height)
{
	tiler2_reservex_nv12(n, width, height, 0, current->tgid);
}
EXPORT_SYMBOL(tiler2_reserve_nv12);
#endif

s32 tiler2_allocx(struct tiler_block_t *blk, enum tiler2_fmt fmt,
				u32 gid, pid_t pid)
{
	struct mem_info *mi;
	struct process_info *pi;
...
EXPORT_SYMBOL(tiler2_allocx);

s32 tiler2_alloc(struct tiler_block_t *blk, enum tiler2_fmt fmt)
{
	return tiler2_allocx(blk, fmt, 0, current->tgid);
}
EXPORT_SYMBOL(tiler2_alloc);

s32 tiler2_mapx(struct tiler_block_t *blk, enum tiler2_fmt fmt, u32 gid,
...
EXPORT_SYMBOL(tiler2_mapx);

s32 tiler2_map(struct tiler_block_t *blk, enum tiler2_fmt fmt, u32 usr_addr)
{
	return tiler2_mapx(blk, fmt, 0, current->tgid, usr_addr);
}
EXPORT_SYMBOL(tiler2_map);

s32 tiler2_mmap_blk(struct tiler_block_t *blk, u32 offs, u32 size,
				struct vm_area_struct *vma, u32 voffs)
{
	...
	BUG_ON(offs > offs + size || offs + size > tiler2_size(blk));
...
EXPORT_SYMBOL(tiler2_mmap_blk);

s32 tiler2_ioremap_blk(struct tiler_block_t *blk, u32 offs, u32 size,
				u32 addr, u32 mtype)
{
	u32 v, p;
	u32 len;		/* area to map */
...
EXPORT_SYMBOL(tiler2_ioremap_blk);

void tiler2_free(struct tiler_block_t *blk)
{
	/* find block */
