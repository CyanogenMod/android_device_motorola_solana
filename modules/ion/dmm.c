...
s32 dmm2_pat_refill(struct dmm *dmm, struct pat *pd, enum pat_mode mode)
{
	void __iomem *r;
	u32 v;
...
EXPORT_SYMBOL(dmm2_pat_refill);

struct dmm *dmm2_pat_init(u32 id)
{
	u32 base;
	struct dmm *dmm;
	switch (id) {
 ...
 */
void dmm2_pat_release(struct dmm *dmm)
{
	if (dmm) {
		iounmap(dmm->base);
		kfree(dmm);
...
EXPORT_SYMBOL(dmm2_pat_release);
