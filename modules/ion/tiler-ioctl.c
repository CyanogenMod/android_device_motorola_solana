...
static s32 tiler_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	for (i = 0; i < b->num_blocks; i++, blk_offs += blk_size) {
		blk_size = tiler2_size(&_b->mi[i]->blk);
		...
		/* mmap block */
		if (tiler2_mmap_blk(&_b->mi[i]->blk, map_offs, map_size, vma,
				   mapped_size))
			return -EAGAIN;
...
s32 tiler2_reg_notifier(struct notifier_block *nb)
{
	if (!nb)
		return -EINVAL;
	return blocking_notifier_chain_register(&notifier, nb);
}
EXPORT_SYMBOL(tiler2_reg_notifier);

s32 tiler2_unreg_notifier(struct notifier_block *nb)
{
	if (!nb)
		return -EINVAL;
	return blocking_notifier_chain_unregister(&notifier, nb);
}
EXPORT_SYMBOL(tiler2_unreg_notifier);
