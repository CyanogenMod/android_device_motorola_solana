...
static void tmm_pat_deinit(struct tmm2 *tmm)
{
	struct fast *f, *f_;
...
static u32 *tmm_pat_get_pages(struct tmm2 *tmm, u32 n)
{
	struct mem *m;
	struct fast *f;
	struct dmm_mem *pvt = (struct dmm_mem *) tmm->pvt;
...
static void tmm_pat_free_pages(struct tmm2 *tmm, u32 *page_list)
{
	struct dmm_mem *pvt = (struct dmm_mem *) tmm->pvt;
	struct fast *f, *f_;
...
static s32 tmm_pat_pin(struct tmm2 *tmm, struct pat_area area, u32 page_pa)
{
	struct dmm_mem *pvt = (struct dmm_mem *) tmm->pvt;
	...
	return dmm2_pat_refill(pvt->dmm, &pat_desc, MANUAL);
}

static void tmm_pat_unpin(struct tmm2 *tmm, struct pat_area area)
{
...
struct tmm2 *tmm2_pat_init(u32 pat_id, u32 *dmac_va, u32 dmac_pa)
{
	struct tmm2 *tmm = NULL;
	struct dmm_mem *pvt = NULL;

	struct dmm *dmm = dmm2_pat_init(pat_id);
	if (dmm)
	...
	kfree(pvt);
	kfree(tmm);
	dmm2_pat_release(dmm);
	return NULL;
}
EXPORT_SYMBOL(tmm2_pat_init);
